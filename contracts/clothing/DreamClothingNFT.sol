...
/**
 * @title Dream clothing NFT a.k.a. parent NFTs
 * @dev Issues ERC-721 tokens as well as being able to hold child 1155 tokens
 */
contract DreamClothingNFT is
    ERC721("DreamClothingNFT", "DreamC"),
    ERC1155Receiver,
    IERC998ERC1155TopDown
{
    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event DreamClothingNFTContractDeployed();

    // @notice event emitted when token URI is updated
    event DreamClothingTokenUriUpdate(
    ...
    mapping(uint256 => address) public clothingDesigners;
    ...
    /**
     @param _accessControls Address of the Dream access control contract
     @param _childContract ERC1155 the Dream child NFT contract
     */
    constructor(DreamAccessControls _accessControls, ERC1155 _childContract)
        public
    {
        accessControls = _accessControls;
        childContract = _childContract;
        emit DreamClothingNFTContractDeployed();
    }

    /**
     @notice Mints a DreamClothingNFT AND when minting to a contract checks if the beneficiary is a 721 compatible
     ...
     @param _designer clothing designer - will be required for issuing royalties from secondary sales
    ...
    function mint(
        ...
        require(
            ...
            "DreamClothingNFT.mint: Sender must have the minter or contract role"
        );
        ...
        // Associate clothing designer
        clothingDesigners[tokenId] = _designer;

        return tokenId;
    }

    /**
     @notice Burns a DreamClothingNFT, releasing any composed 1155 tokens held by the token itseld
    ...
    function burn(uint256 _tokenId) external {
        ...
        require(
            ...
            "DreamClothingNFT.burn: Only clothing owner or approved"
        );
        ...
        delete clothingDesigners[_tokenId];
    ...
    function onERC1155Received(
        ...
        require(
            parentToChildMapping[_receiverTokenId].length() <=
                maxChildrenPerToken,
    ...
    function _validateReceiverParams(
        uint256 _receiverTokenId,
        address _operator,
        address _from
    ) internal view {
        ...
        // We only accept children from the Dream child contract
        require(
            _msgSender() == address(childContract),
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external {
        require(
            ...
            "DreamClothingNFT.setTokenURI: Sender must be an authorised contract or admin"
        );
        _setTokenURI(_tokenId, _tokenUri);
        emit DreamClothingTokenUriUpdate(_tokenId, _tokenUri);
    }
     ...
     */
    function setPrimarySalePrice(uint256 _tokenId, uint256 _salePrice)
        external
    {
        require(
            accessControls.hasSmartContractRole(_msgSender()) ||
                accessControls.hasAdminRole(_msgSender()),
            "DreamClothingNFT.setPrimarySalePrice: Sender must be an authorised contract or admin"
        );
        require(
            _exists(_tokenId),
            "DreamClothingNFT.setPrimarySalePrice: Token does not exist"
        );
        require(
            _salePrice > 0,
            "DreamClothingNFT.setPrimarySalePrice: Invalid sale price"
        );
        ...
        external
    {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "DreamClothingNFT.updateAccessControls: Sender must be admin"
        );
        accessControls = _accessControls;
    }
    ...
    function updateMaxChildrenPerToken(uint256 _maxChildrenPerToken) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "DreamClothingNFT.updateMaxChildrenPerToken: Sender must be admin"
        );
    ...
    function childBalance(
        ...
        return
            _childContract == address(childContract)
                ? balances[_tokenId][_childTokenId]
                : 0;
    }
     ...
     */
    function childContractsFor(uint256 _tokenId)
        external
        view
        override
     ...
     */
    function childIdsForOn(uint256 _tokenId, address _childContract)
        public
        view
        override
        returns (uint256[] memory)
    {
        ...
        uint256[] memory childTokenIds = new uint256[](
            parentToChildMapping[_tokenId].length()
        );
     ...
     */
    function totalChildrenMapped(uint256 _tokenId)
        external
        view
        returns (uint256)
    {
    ...
    function _extractAndTransferChildrenFromParent(
        uint256 _fromTokenId,
        address _to
    ) internal {
        uint256[] memory _childTokenIds = childIdsForOn(
            _fromTokenId,
        ...
        childContract.safeBatchTransferFrom(
            address(this),
            _to,
            _childTokenIds,
            _amounts,
    ...
    function _receiveChild(
        ...
        balances[_tokenId][_childTokenId] = balances[_tokenId][_childTokenId]
            .add(_amount);
    }
    ...
    function _removeChild(
        ...
        require(
            _amount != 0 || balances[_tokenId][_childTokenId] >= _amount,
    ...
    function _assertMintingParamsValid(
        ...
        require(
            ...
            "DreamClothingNFT._assertMintingParamsValid: Token URI is empty"
        );
        require(
            _designer != address(0),
            "DreamClothingNFT._assertMintingParamsValid: Designer is zero address"
        );
    }
}
