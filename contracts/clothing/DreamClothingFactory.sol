 ...
 */
contract DreamClothingFactory is Context {
    // @notice event emitted on garment creation
    event GarmentCreated(uint256 indexed garmentTokenId);

    // @notice the parent ERC721 garment token
    DreamClothingNFT public garmentToken;
    ...
    constructor(
        DreamClothingNFT _garmentToken,
     ...
     */
    function createNewChild(string calldata _uri)
        external
        returns (uint256 childTokenId)
    {
        require(
            accessControls.hasMinterRole(_msgSender()),
            "DreamClothingFactory.createNewChild: Sender must be minter"
        );
     ...
     */
    function createNewChildren(string[] calldata _uris)
        external
        returns (uint256[] memory childTokenIds)
    {
        require(
            accessControls.hasMinterRole(_msgSender()),
            "DreamClothingFactory.createNewChildren: Sender must be minter"
        );
    ...
    function mintParentWithChildren(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "DreamClothingFactory.mintParentWithChildren: Sender must be minter"
        );
        // Generate parent 721 token
        uint256 garmentTokenId = garmentToken.mint(
            beneficiary,
            garmentTokenUri,
            designer
        );
        ...
        materials.batchMintChildren(
            childTokenIds,
            childTokenAmounts,
            address(garmentToken),
            abi.encodePacked(garmentTokenId)
        );
    ...
    function mintParentWithoutChildren(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "DreamClothingFactory.mintParentWithoutChildren: Sender must be minter"
        );
